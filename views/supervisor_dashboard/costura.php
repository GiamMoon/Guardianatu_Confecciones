...
if (!empty($tareas)) {
    ...
    <style>
        ...
        .completado {
            color: green; /* Cambia el color a verde para "completado" */
        }
    </style>
    ...
    <?php if (!empty($tareasAgrupadas)) : ?>
        <?php 
            foreach ($tareasAgrupadas as $clienteId => $infoCliente) : ?>
            <div class="cliente">
    ...
    <li>
        ...
        <div>
            <p class="nombre-tarea">
                Estado: <span class="<?php echo ($tarea->estadoCostura == 0) ? 'pendiente' : 'completado'; ?>">
                    <?php echo ($tarea->estadoCostura == 0) ? 'Pendiente' : 'Completado'; ?>
                </span>
            </p>
        </div>
        <!-- Acceder a los datos del cliente -->
    </li>
    <hr> <!-- Agrega una línea horizontal después de cada tarea -->
<?php endforeach; ?>
