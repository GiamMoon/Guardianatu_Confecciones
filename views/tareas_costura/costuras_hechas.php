...
<script>
    ...
    function manejarBotonesEstado() {
        ...
        botonesEstado.forEach((boton) => {
            boton.addEventListener("click", async function () {
                ...
                // Cambiar el valor de estadoCostura según el nuevo estado
                const nuevoEstadoCostura = nuevoEstado === "0" ? 1 : 0;
                ...
                await actualizarEstadoEnServidor(tareaId, nuevoEstadoCostura, usuarioId);
                ...
                // Opcional: Actualiza el estado en el DOM sin recargar la página
                const estadoTexto = nuevoEstado === "0" ? "Pendiente" : "Realizado";
                boton.closest(".tarea").querySelector("span").innerText = estadoTexto;
    ...
    async function actualizarEstadoEnServidor(tareaId, nuevoEstado) {
        try {
            // Obtener el ID del usuario de sesión
            const usuarioId = <?php echo json_encode($_SESSION['id'] ?? null); ?>;
            const estadoCostura = 0; // Cambiar esto según tu lógica
